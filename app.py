...
# from processor import process_harmonization
...
#     #Get HD
#     hd_enabled = request.form.get('hd_mode') == 'true'

#     # CHECKBOX: Get SMART mode (returns 'true' if checked)
...
#     try:
#         db.session.commit()
#         print("Databbase committed successfully...")
#     except Exception as e:
#         db.session.rollback()
...
# @app.route('/results')
# def results():
#     """Fetches latest 20 results from the database to display in the gallery."""
#     # Fetch results from database ordered by newest first
#     all_results = ResultImage.query.order_by(ResultImage.timestamp.desc()).all()
